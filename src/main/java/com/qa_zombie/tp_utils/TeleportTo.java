	...
	public static void execute(CommandContext<CommandSourceStack> arguments, Entity entity) {
		...
		if (ZombieSettingsConfiguration.TELEPORT_REQUEST_FUNCTION.get()) {
			if ((commandParameterEntity(arguments, "player")) instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal(((commandParameterEntity(arguments, "player")).getDisplayName().getString() + "" + Component.translatable("qa_zombie_util.info.tp_to").getString())), false);
			{
				...
				if (!_ent.level().isClientSide() && _ent.getServer() != null) {
					_ent.getServer().getCommands().performPrefixedCommand(
							...
							("tellraw @s {\"text\":\"" + "" + Component.translatable("qa_zombie_util.info.agree").getString()
									+ "\",\"color\":\"blue\",\"bold\":true,\"clickEvent\":{\"action\":\"run_command\",\"value\":\"/zombie_utli_teleport_to agree " + entity.getDisplayName().getString() + "\"}}"));
				}
			}
			(commandParameterEntity(arguments, "player")).getPersistentData().putDouble("agreeTick", 200);
			(commandParameterEntity(arguments, "player")).getPersistentData().putString("agreePlayer", (entity.getDisplayName().getString()));
		} else {
